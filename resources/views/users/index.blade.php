                        ...
                        @forelse($user as $item)
                            <tr>
                                ...
                                <td class="border px-6 py- text-center">
                                    <a href="{{ route('users.edit', $item->id) }}" class="bg-blue-400 hover:bg-blue-700 text-white font-bold py-1 px-4 inline-block rounded">
                                        Edit
                                    </a>
                                    <form action="{{ route('users.destroy', $item->id) }}" method="POST" class="inline-block">
                                        {!! method_field('delete') . csrf_field() !!}
                                        <button type="submit" class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-4 mx-2 rounded inline-block">
                                            Delete
                                        </button>
                                    </form>
                                </td>
